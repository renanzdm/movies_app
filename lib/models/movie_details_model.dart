...
class MovieDetailsModel {
  ...
  factory MovieDetailsModel.fromMap(Map<String, dynamic> map) {
    ...
    var urlImagesPath = urlImagesPosters?.map<String>((image)=>'https://image.tmdb.org/t/p/w500${image['file_path']}').toList() ?? [];

    return MovieDetailsModel(
      title: map['title'] ?? '',
      stars: map['vote_average'] ?? 0.0,
      genres: List<GenresModel>.from(map['genres']?.map((x) => GenresModel.fromMap(x)) ?? const []),
      imagesUrl: urlImagesPath,
      releaseDate: DateTime.parse(map['release_date']),
      overview: map['overview'] ?? '',
      productionsCompany: List<dynamic>.from(map['production_companies'] ?? const []).map<String>((e) => e['name']).toList(),
